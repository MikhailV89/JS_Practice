    ...
    for (let i = 0; i < categories.length; i++) {
        ...
        for (let j = 0; j < categories[i].products.length; j++) {
            ...
            for (let k = 0; k < sales.length; k++) {
                if (sales[k].productId === categories[i].products[j].id) {
                    const subLi = document.createElement('li')
                    subLi.textContent = `${sales[k].qty} - ${sales[k].status}`
                    subUl.append(subLi)
                }
            }
...
function createListElement (contentLi, contentSpan) {
    const elementLi = document.createElement('li')
    elementLi.textContent = contentLi

    const elementSpan = document.createElement('span')
    elementSpan.textContent = contentSpan

    elementLi.append(elementSpan)
    elementLi.append(createButton())

    return elementLi
}

for (let i = 0; i < teams.length; i++) {
  const li = createListElement(`${teams[i].title}: `, `${teams[i].playersCount} players, ${teams[i].totalGoals} goals` )

    const innerUl = document.createElement('ul')

    for (let j = 0; j < teams[i].players.length; j++) {
         const innerLi = createListElement(`${teams[i].players[j].name}:`, `${teams[i].players[j].goals} goals` )

        const subUl = document.createElement('ul')
        for (let k = 0; k < matches.length; k++) {
            if (matches[k].playerId === teams[i].players[j].id) {
                const subLi = document.createElement('li')
                subLi.textContent = `${matches[k].goals} - ${matches[k].status}`
                subUl.append(subLi)
            }
        }

        // innerLi.append(createButton())
        innerLi.append(subUl)
        innerUl.append(innerLi)
    }
    li.append(innerUl)
